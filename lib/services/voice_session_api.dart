...
class VoiceSessionApi {
  ...
  Future<VoiceSessionInfo> startSession(String userId) async {
    final url = Uri.parse('$baseUrl/api/token/generate');
    ...
    try {
      debugPrint('[VoiceSessionApi] Generating token for channel: $channelName, uid: $userUid');
      ...
      if (tokenResponse.statusCode != 200) {
        throw Exception(
            'Failed to generate token: ${tokenResponse.statusCode} ${tokenResponse.body}');
      }
